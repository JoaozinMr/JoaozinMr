<img src="https://raw.githubusercontent.com/MicaelliMedeiros/micaellimedeiros/master/image/computer-illustration.png" min-width="400px" max-width="400px" width="400px" align="right" alt="Computer Illustration">

<p align="left">
  Hi, I'm <strong>João Gabriel</strong> — <strong>FullStack Developer & Software Analyst</strong>.<br>
  Currently pursuing a B.S. in Computer Science at Faculdade Cotemig (Class of 2027).
</p>

<p align="left">
  Driven by building scalable applications and exploring <strong>Software Architecture (DDD)</strong> and <strong>Clean Code</strong>.
</p>

<p align="left">
  💻 Languages: <strong>C#, JavaScript, TypeScript, PHP, Ruby, SQL (MySQL, PostgreSQL, Oracle).</strong>
</p>

<p align="left">
  💼 Frameworks & Tools: <strong>.NET, Node.js, React, Next.js, Laravel, Ruby on Rails, Docker, AWS.</strong>
</p>

<p align="left">
  💌 Reach out: ⤵️
</p>

<p align="left">
  <a href="mailto:joao.florv2005@gmail.com" alt="Gmail">
  <img src="https://img.shields.io/badge/-Gmail-FF0000?style=flat-square&labelColor=FF0000&logo=gmail&logoColor=white" /></a>

  <a href="https://www.linkedin.com/in/jo%C3%A3o-gabriel-vieira-val%C3%A9rio-flor-663309231/" alt="Linkedin">
  <img src="https://img.shields.io/badge/-Linkedin-0e76a8?style=flat-square&logo=Linkedin&logoColor=white" /></a>

  <a href="https://wa.me/5531971045343" alt="WhatsApp">
  <img src="https://img.shields.io/badge/-WhatsApp-25d366?style=flat-square&labelColor=25d366&logo=whatsapp&logoColor=white"/></a>
</p>

---

<div>
<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=JoaozinMr&layout=compact&langs_count=7&theme=dracula"/>
<img height="180em" src="https://github-readme-stats.vercel.app/api?username=JoaozinMr&show_icons=true&theme=dracula&include_all_commits=true&count_private=true"/>
</div>
